...
const claim = async () => {
    const tx = await emissionsContract.claim($signerAddress, ethers.constants.AddressZero)
...
<div class="w-full max-w-prose flex flex-col gap-y-4">
    ...
    {#if !params.wild}
    <FormPanel>
        <span class="text-gray-400">Enter an EmissionsERC20 contract address below.</span>
        <Input
    ...
    <FormPanel heading="Claim">
        ...
        <div class="flex flex-col gap-y-4">
            <span class="text-gray-400">Calculate claim for {$signerAddress}</span>
            <Button
                on:click={() => {
        ...
        <div>
            ...
            {:then claim}
            Your claim will be {formatUnits(claim, token.erc20decimals)} {token.erc20symbol}
            {/await}
        </div>
        {/if}
        ...
        <Button shrink on:click={()=>{claimPromise = claim()}}>Claim</Button>
        {#if claimPromise}
            {#await claimPromise}
                Claiming...
            {:then}
